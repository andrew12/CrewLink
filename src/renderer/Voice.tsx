...
function calculateVoiceAudio(
  ...
  if (
    ...
    (state.gameState === GameState.LOBBY && !settings.stereoInLobby)
  ) {
    panPos = [0, 0];
  }
  ...
  if (
    gain.gain.value === 1 &&
    Math.sqrt(Math.pow(me.x - other.x, 2) + Math.pow(me.y - other.y, 2)) > 7
  ) {
    gain.gain.value = 0;
  }
}
...
export default function Voice() {
  ...
  useEffect(() => {
    ...
    connectionStuff.current.socket = io(`ws://${settings.serverIP}`, {
    ...
    navigator.getUserMedia(
      ...
      async (stream) => {
        ...
        ipcRenderer.on("toggleDeafen", () => {
          ...
          stream.getAudioTracks()[0].enabled = !connectionStuff.current
            .deafened;
