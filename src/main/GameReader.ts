...
export default class GameReader {
  ...
  checkProcessOpen(): void {
    let processOpen = getProcesses().find(
  ...
  loop(): void {
    ...
    if (this.amongUs !== null && this.gameAssembly !== null) {
      ...
      let meetingHud = this.readMemory<number>(
        "pointer",
        this.gameAssembly.modBaseAddr,
        this.offsets.meetingHud
      );
      let meetingHud_cachePtr =
        meetingHud === 0
          ? 0
      ...
      let gameState = this.readMemory<number>(
        "int",
        this.gameAssembly.modBaseAddr,
      ...
      let allPlayers = this.readMemory<number>(
        "ptr",
        allPlayersPtr,
        this.offsets.allPlayers
      );
      let playerCount = this.readMemory<number>(
        "int" as "int",
        allPlayersPtr,
      ...
      let exiledPlayerId = this.readMemory<number>(
        "byte",
        this.gameAssembly.modBaseAddr,
      ...
      for (let i = 0; i < Math.min(playerCount, 10); i++) {
        let { address, last } = this.offsetAddress(
          playerAddrPtr,
          this.offsets.player.offsets
        );
        let playerData = readBuffer(
          this.amongUs.handle,
          address + last,
  ...
  readString(address: number): string {
    if (address === 0) return "";
    let length = readMemoryRaw<number>(
      this.amongUs!.handle,
      address + 0x8,
      "int"
    );
